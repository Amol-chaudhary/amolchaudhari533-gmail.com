...
while(1):
    ...
    if(ch==2):
        ...
        q1 = "select acc,Account_number from account_login where Account_number=%s and pass=%s"
        values = [x,y]
        ...
        if(b2!=None):
            ...
            while(1):
                ...
                if(c==2):
                    ...
                    if(a2!=None):
                        if(total!=0):
                            ...
                            c1.execute(q6,cost3)
                            db.commit()
                            q7 = "insert into transaction values(%s,%s,%s,%s)"
                            val1 = [0,b2[1],ne,k]
                            c1.execute(q7,val1)
                            db.commit()
                            q8 = "insert into transaction values(%s,%s,%s,%s)"
                            val2 = [0,ne,b2[1],k]
                            c1.execute(q8,val2)
                            db.commit()
                        else:
                ...
                if(c==3):
                    ...
                    q11="select *from transaction where from_user_acc=%s"
                    g11=[fm]
                    c1.execute(q11,g11)
                    h1=c1.fetchall()
                    print h1
